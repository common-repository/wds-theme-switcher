...
function pn_get_template_part( $slug, $name = '', $include = true ) {
	...
	// Look in yourtheme/slug-name.php and yourtheme/wds-theme-switcher/slug-name.php
	if ( $name ) {
	...
	// If template file doesn't exist, look in yourtheme/slug.php and yourtheme/wds-theme-switcher/slug.php
	if ( !$template ) {
		$template = locate_template( array( "{$slug}.php", $plugin_slug . "{$slug}.php" ) );
	}
...
function pn_get_email_template( $name, $prefix = '' ) {
	...
	// Look in yourtheme/wds-theme-switcher/{locale}/name.tpl and yourtheme/wds-theme-switcher/email-templates/{locale}/name.tpl
	if ( empty( $template ) ) {
		$search = array(
